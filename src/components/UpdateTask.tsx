import './UpdateTask.css';
import { useState } from "react";
import { useContext } from "react";
import {ProjectsTasksContext} from "../store/projects-tasks-context";
import { TTask } from '../types';
import Button from './Button.tsx';

interface IUpdateTaskProps {
    task: TTask,
}

export default function UpdateTaskItem({task}: IUpdateTaskProps) {

    const [newText, setText] = useState(task.description);
    const data = useContext(ProjectsTasksContext);
    
    function handleChange(event) {
        setText(event.target.value);
    }

    function onEnter(event) {
        if(event.key === 'Enter') {
            data.updateTaskText(task.uuid, newText)
        }
    }
    
    return(
        <div id="update-task" className='flex-col sm:flex-row sm:items-center gap-2'>
            <div className='input-container'>
                <input type="text" className="text-sm sm:text-base" placeholder="Neue Aufgabe hinzufügen..." value={newText} onChange={handleChange} onKeyDown={onEnter}/>
            </div>
            <div className='btn-container w-full justify-end gap-2 sm:w-auto'>
                <Button onClick={() => data.updateTaskText(task.uuid, newText)} variant='primary' animation="scale" className="w-10 h-10 shrink-0 text-lg rounded-md font-bold shadow-sm">
                        +
                </Button>
                <Button onClick={() => data.cancel(task.uuid)} variant='secondary' animation="scale" className="h-10 px-3 shrink-0 whitespace-nowrap text-sm sm:text-base font-bold rounded-md">
                    Abbrechen
                </Button>

            </div>
        </div>
    )
}
